...
def new_members_pdf_view(request):
    
    # now = datetime.now()
    # that_day = date(2021, 10,24)
    all = User.objects.filter(date_joined__range=["2021-10-01", "2021-11-8"]).all()
    
    html_string = render_to_string('pdf_templates/new_members.html', {'all': all})
