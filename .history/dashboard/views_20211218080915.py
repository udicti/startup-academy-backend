...
def send_activation_link(id):
    ...
    activation_url = reverse('activate', uidb64=urlsafe_base64_encode(force_bytes(user.pk) token=token)
    
    data['email-body'] = [
        f"Dear {user.username}",
        f"Thank you for creating an account in Udictihub, Click the link below to activate your account.",
        f"a> Activation Link href> { domain }{% url 'activate' uidb64=uid token=token %}"
    ]
