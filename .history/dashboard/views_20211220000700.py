...
class ForgotPasswordView(View):
    ...
    def get(self, request):
        return render(request, "dashboar/forgot-password.html", context={'sent':False, 'email':''})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user:
                send_email_for_password_reset(user, redirect_url=f"{SITE}/login/")
                return render(request, "auth/forgot-password.html", context={'sent':True, 'email':email})
            else:
                error = "no user, with such email"
        else:
            error = form.errors.as_text()
        return render(request, "auth/forgot-password.html", context={'sent':False, 'email':'', "error":error})
...
class AddNewPassword(View):
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        if user:
            return render(request, "auth/new-password.html", context={"user":user, "error":None})
        # if not send to the resend link page
        else:
            return render(request, "auth/forgot-password.html", context={'sent':False, 'email':'', "error":"Link has expired, submit your email to send the link again "})
...
def team_member_add(request, user_id):
...
def team_member_remove(request, user_id):
