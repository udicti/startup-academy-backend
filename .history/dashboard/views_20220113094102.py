...
class teams_view(View):
    ...
    def post(self, request):
        ...
        new = Teams(name=name)
        new.save()
        messages.error(request, "Added a new team")
...
def team_view(request, id):
    ...
    def validate_membership_capacity(user: User):
        ...
        if user.is_superuser() or user not in members:
            return True
    ...
    context = {
        ...
        "all_members": [i for i in User.objects.all() if ( and validate_membership_capacity(i) is True)]
    }
