...
def member_view(request, id):
    ...
    try:
        yin = int(profile.year_of_study.year)
        yad = int(profile.admission_date.year)
        tos = profile.study_period
        print(f"Year in {yin} admited {yad} tos {tos}")
        context["time_to_left"] = profile.admission_date.year
    except: 
        pass
