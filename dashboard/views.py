...
def member_view(request, id):
    ...
    try:
        yostudy = profile.year_of_study
        yadmited = profile.admission_date.year
        ytolive = yadmited + profile.study_period
        # print(f" \n\n Year in {yadmited} while in {yostudy} year will live to {ytolive} \n\n")
        context["time_to_left"] = ytolive
        context["time_around"] = ytolive - (yadmited + yostudy)
    except: 
        pass
...
class StartAttendence(View):
    ...
    def post(self, request):
        ...
        event = Event.objects.filter(id=request.POST.get('id')).first()
